...
def main() -> None:
    ...
    DATA_ROOT   = Path(__file__).parent / "DATASET_Processed"
    IMG_SIZE    = 224
    BATCH_SIZE  = 128
    EPOCHS      = 30
    LR          = 3e-4
    ...
    train_folder = datasets.ImageFolder(DATA_ROOT / "train_processed", transform=train_tfms)
    test_folder  = datasets.ImageFolder(DATA_ROOT / "test_processed",  transform=train_tfms)
