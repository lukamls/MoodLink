...
def main() -> None:
    ...
    DATA_ROOT   = Path(__file__).parent / "archive"
    IMG_SIZE    = 224
    BATCH_SIZE  = 128
    EPOCHS      = 30
    LR          = 3e-4
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_loss < best_val:
            torch.save(model.state_dict(), "best_mobilenetv3_fer13.pth")
            best_val = val_loss
            epochs_no_improve = 0
        else:
            epochs_no_improve += 1
    ...
    model.load_state_dict(torch.load("best_mobilenetv3_fer13.pth"))
    model.eval()
    y_true, y_pred = [], []
    ...
    torch.onnx.export(
        model, dummy, "mobilenetv3_fer13.onnx",
        input_names=['input'], output_names=['output'],
        dynamic_axes={'input': {0: 'batch'}, 'output': {0: 'batch'}},
        opset_version=12
    )
    print("ONNX model saved ➜ mobilenetv3_fer13.onnx")
